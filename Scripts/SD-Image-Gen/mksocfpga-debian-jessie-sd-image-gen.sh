...
apt -y install apt-utils sudo
apt -y install adduser openssh-client openssh-server openssl nano locales

apt -y install resolvconf apt-utils ssh ntpdate openssl nano locales libssh2-1 kmod dbus dbus-x11 xorg xserver-xorg-video-dummy upower rsyslog udev libpam-systemd systemd-sysv net-tools lsof less accountsservice iputils-ping python ifupdown iproute2 dhcpcd5 avahi-daemon uuid-runtime avahi-discover libnss-mdns debianutils traceroute strace
